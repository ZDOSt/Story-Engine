...
export const STREAMING_ARTIFACT_REGEX_SOURCE = [
    ...
    '^\\s*(?:\\d+[.)]?\\s*(?:[*_~]{1,3})?\\s*(?:smellGate|olfactoryGate|abilityAndItemRender|abilityIntegration|epistemicRender|behavioralRender|literalStyleFilter|sceneBeatComposition|chronologyControl|userAgencyControl|turnStructureControl|responseEndpointControl|turnBoundaryControl)\\b[\\s\\S]*?)(?=\\n\\s*(?!\\d+[.)]?\\s*(?:[*_~]{1,3})?\\s*(?:smellGate|olfactoryGate|abilityAndItemRender|abilityIntegration|epistemicRender|behavioralRender|literalStyleFilter|sceneBeatComposition|chronologyControl|userAgencyControl|turnStructureControl|responseEndpointControl|turnBoundaryControl)\\b)(?!(?:[*_~]{1,3})?\\s*(?:CONCLUSION|VALIDATION CONCLUSION|FINAL CHECK|RENDER CHECK)\\s*:)|BEGIN_FINAL_NARRATION|$)',
