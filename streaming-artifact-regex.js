...
export const STREAMING_ARTIFACT_REGEX_SOURCE = [
    ...
    '^\\s*(?:\\d+[.)]?\\s*(?:[*_~]{1,3})?\\s*(?:olfactoryGate|abilityIntegration|epistemicRender|behavioralRender|literalStyleFilter|sceneBeatComposition|turnBoundaryControl)\\b[\\s\\S]*?)(?=\\n\\s*(?!\\d+[.)]?\\s*(?:[*_~]{1,3})?\\s*(?:olfactoryGate|abilityIntegration|epistemicRender|behavioralRender|literalStyleFilter|sceneBeatComposition|turnBoundaryControl)\\b)(?!(?:[*_~]{1,3})?\\s*(?:CONCLUSION|VALIDATION CONCLUSION|FINAL CHECK|RENDER CHECK)\\s*:)|BEGIN_FINAL_NARRATION|$)',
