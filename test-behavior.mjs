...
const tests = [
  ...
  },
  {
    ...
    run() {
      ...
      assert.match(text, /attemptedActionResults:\nA1: SUCCESS - Narrate this listed action succeeding narrowly, with limited visible impact\.\nA2: FAILURE - This listed action does not land or remains unfinished\.\nA3: FAILURE - This listed action does not land or remains unfinished\./);
      ...
      const report = runCase({
        ...
        ledger: baseLedger({
          resolutionEngine: {
            ...
            actionBucket: 'Combat',
            socialBucket: 'None',
            combatType: 'SpellOrSupernatural',
          },
  ...
  },
  {
    ...
    run() {
      ...
      assert.equal(resolution.OutcomeTier, 'Critical_Success');
      assert.equal(resolution.LandedActions, 1);
      assert.match(prompt(report), /attemptedActionResults:\nA1: SUCCESS - Narrate this listed action succeeding decisively\./);
  ...
  },
  {
    ...
    run() {
      ...
      assert.equal(auditIncludes(report, 'bucketStats={USER:CHA,OPP:CHA'), true);
  ...
  },
  {
    ...
    run() {
      ...
      assert.match(prompt(report), /speech style and interaction style carry most personality/i);
      assert.match(prompt(report), /mannerism is optional, flexible, and scene-valid only/i);
      assert.match(prompt(report), /do not repeat it every response, force props\/locations, or turn it into a required beat/i);
    },
  },
  {
    ...
    run() {
      ...
      assert.match(TRACKER_DELTA_CONTRACT, /one optional flexible, scene-valid habit/);
      assert.match(TRACKER_DELTA_CONTRACT, /not a fixed gesture, prop, location, or mandatory repeated beat/);
  ...
  },
  {
    ...
    run() {
      ...
      const renderControlLeak = [
        '1. **embodiedPerception:** No explicit smell/taste invocation. No overpowering close-range source. -> LOCKED. No smell/taste.',
        '',
        '2. **diegeticPhysicality:** No active abilities or supernatural traits are being used.',
        '',
        '3. **strictEpistemology:** Direct visual scene only.',
        '',
        '4. **behaviorism:** No body-tell shorthand.',
        '',
        '5. **denotativePhysicality:** Keep narration literal.',
        '',
        '6. **hypotacticSceneBeats:** Advance the visible scene.',
        '',
        '7. **linearChronology:** Begin with the immediate consequence after the user input.',
        '',
        '8. **agencySeparation:** Do not write user action.',
        '',
        '9. **activeHandoff:** Stop at the user handoff point.',
        '',
        '10. **activeHandoff:** Stop when the NPC response reaches the user.',
        '',
  ...
  },
  {
    ...
    run() {
      ...
      assert.match(preflightSource, /Render abilities and unusual effects through observable consequences only\./i);
      assert.match(preflightSource, /Do not name or explain powers unless spoken in dialogue\./i);
  ...
  },
  {
    ...
    run() {
      ...
      assert.match(preflightSource, /Obey listed personal item availability exactly\./i);
      assert.match(preflightSource, /Unavailable items must not appear possessed or usable\./i);
  ...
  },
  {
    ...
    run() {
      ...
      assert.match(enginesSource, /Bluff for deception, lying, false claims, misleading, tricking/i);
      assert.match(enginesSource, /Bluff\/Intimidate=CHA vs MND/);
  ...
  },
  {
    ...
    run() {
      ...
      const text = prompt(report);
      assert.match(text, /FAILED NPC ATTACK: Ogre: proactive attack from current hostile state against Seraphina fails/i);
      assert.match(text, /does not land, connect, injure, restrain, shove, control, produce a completed spell effect, or create any completed forceful effect/i);
  ...
  },
  {
    ...
    run() {
      ...
      const text = prompt(report);
      assert.match(text, /FAILED NPC ATTACK: Valerie: proactive attack from current hostile state against \{\{user\}\} fails/i);
      assert.match(text, /does not land, connect, injure, restrain, shove, control, produce a completed spell effect, or create any completed forceful effect/i);
    },
  },
  {
    ...
    run() {
      ...
      const text = prompt(report);
      assert.match(text, /CONTESTED NPC ATTACK: Valerie: proactive attack from current hostile state against \{\{user\}\} remains unresolved/i);
      assert.match(text, /does not produce a completed hit, injury, restraint, shove, control, spell effect, or forceful effect/i);
    },
  },
  {
  ...
  },
  {
    ...
    run() {
      ...
      assert.doesNotMatch(source, /Ã¢â€ |ÃƒÂ¢/);
      ...
      const sectionDefaults = source.slice(
        ...
        source.indexOf('const DEFAULT_PROSE_GUARD_FORMATTING_PROMPT'),
      );
      assert.match(sectionDefaults, /During dialogue or conversation, keep the narration close to the participants/);
      ...
      assert.match(sectionDefaults, /Environmental detail within dialogue or conversation belongs only when it is directly relevant/);
      ...
      assert.match(profileBuilder, /sceneStyleProfile\(response, context\):/);
      ...
      assert.match(source, /injectMovablePrompt\(\s*WRITING_STYLE_PROMPT_KEY,\s*promptText,\s*'before_prompt',\s*0,\s*EXTENSION_PROMPT_ROLES\.SYSTEM,\s*\)/);
      assert.match(source, /injectMovablePrompt\(\s*PROSE_RULES_PROMPT_KEY,\s*DEFAULT_PROSE_RULES_PROMPT,\s*'before_prompt',\s*0,\s*EXTENSION_PROMPT_ROLES\.SYSTEM,\s*\)/);
      assert.doesNotMatch(source, /injectMovablePrompt\(\s*(?:WRITING_STYLE_PROMPT_KEY|PROSE_RULES_PROMPT_KEY),[\s\S]{0,120}'in_prompt'/);
  ...
  },
  {
    ...
    run() {
      const indexSource = fs.readFileSync(new URL('index.js', import.meta.url), 'utf8');
      const handoffSource = fs.readFileSync(new URL('pre-flight.js', import.meta.url), 'utf8');
      assert.match(indexSource, /Render character state through behaviorism: observable behavior and physical displacement/);
      assert.match(indexSource, /tangible, external action that could be noticed by someone present in the scene/);
      assert.match(indexSource, /canned body-language shorthand, autonomic emotional tropes/);
      assert.match(indexSource, /Never use blushing, flushing, reddening, paling, or knuckle-whitening as emotional shorthand/);
      assert.match(indexSource, /Strict literalism: keep prose physically clear and grounded in direct scene evidence/);
      assert.match(indexSource, /Use literal language when describing scenes/);
      assert.match(indexSource, /policy: COHESIVE-ACTION-RESULT/);
      assert.match(indexSource, /Hypotactic narration: write cohesive scene beats/);
      assert.match(indexSource, /Avoid paratactic narration/);
      ...
      assert.match(indexSource, /micro-reaction loops, twitch-cadence narration, or body-cue pileups/);
      ...
      assert.match(handoffSource, /Render character state through observable behavior and physical displacement\./);
      assert.match(handoffSource, /Do not use emotion labels, canned body-language shorthand, or repeated micro-gestures\./);
      assert.match(handoffSource, /Do not break one beat into staccato action stacking or body-cue pileups\./);
      assert.doesNotMatch(handoffSource, /breath\/throat\/pulse\/stomach cues/);
      assert.doesNotMatch(handoffSource, /skin-color emotional shorthand/);
      ...
      const indexCharacterTurnPacing = indexSource.slice(
        indexSource.indexOf('  characterTurnPacing(response, context):'),
        indexSource.indexOf('applicationContract:'),
      );
      assert.match(indexCharacterTurnPacing, /policy: BOUNDED-CHARACTER-TURNS/);
      assert.match(indexCharacterTurnPacing, /1 continuous, uninterrupted paragraph containing one NPC's complete contribution/);
      assert.match(indexCharacterTurnPacing, /paragraph must be 1-6 sentences long/);
      assert.match(indexCharacterTurnPacing, /address multiple connected points from \{\{user\}\}/);
      assert.match(indexCharacterTurnPacing, /one coherent turn/);
      assert.match(indexCharacterTurnPacing, /same one-paragraph limit/);
      assert.match(indexCharacterTurnPacing, /brief NPC A -> NPC B -> NPC A exchange/);
      assert.match(indexCharacterTurnPacing, /Do not give the same NPC a second paragraph, second turn/);
      assert.doesNotMatch(indexCharacterTurnPacing, /Surrounding scene detail/);
      assert.doesNotMatch(indexCharacterTurnPacing, /current interaction and active participants/);
    },
  },
  {
    ...
    run() {
      const indexSource = fs.readFileSync(new URL('index.js', import.meta.url), 'utf8');
      const handoffSource = fs.readFileSync(new URL('pre-flight.js', import.meta.url), 'utf8');
      assert.match(indexSource, /policy: ACTIVE-HANDOFF/);
      assert.match(indexSource, /PLAN your response so that it ends on a natural beat that \{\{user\}\} can immediately respond to/);
      assert.match(indexSource, /Write toward that beat, and once it is reached, your response ENDS/);
      assert.match(indexSource, /Do not continue past the handoff beat/);
      assert.match(indexSource, /Do not end on explicit waiting, staring, mood-only silence, or all-eyes-on-user framing/);
      assert.match(indexSource, /policy: ZERO-ECHO, IMMEDIATE-CONSEQUENCE/);
      assert.match(indexSource, /Narrate in strict linear order\. Begin with the immediate consequence of \{\{user\}\}'s latest input/);
      assert.match(indexSource, /Do NOT echo, summarize, recap, paraphrase, or repeat any part of \{\{user\}\}'s actions or dialogue/);
      assert.match(indexSource, /Do NOT jump ahead, rewind, or insert undeclared intermediate actions/);
      assert.match(indexSource, /policy: USER-CONTROL-BOUNDARY/);
      assert.match(indexSource, /The narrator controls the world, NPCs, hazards, objects, and consequences/);
      assert.match(indexSource, /Render \{\{user\}\}'s voluntary actions only when the latest input explicitly declares them/);
      assert.match(indexSource, /External physical forces may affect \{\{user\}\} when concrete and proportional/);
      assert.match(indexSource, /Do not write \{\{user\}\} speech, thoughts, feelings, choices/);
      ...
      assert.match(handoffSource, /#1 - RESOLVED FACTS/);
      assert.match(handoffSource, /For this response, narrativeFacts\(input\) is the authoritative scene resolution and the only source of resolved outcomes\./);
      assert.match(handoffSource, /Do not contradict, soften, reverse, override, or invent additional resolved outcomes beyond it\./);
      assert.match(handoffSource, /Authoritative resolved scene facts for this response\./);
      assert.match(handoffSource, /#2 - PROSE RULES/);
      assert.match(handoffSource, /Execute renderControlEngine\(input\) as a hard constraint on the response\./);
      assert.match(handoffSource, /Narrate through concrete physical evidence from \{\{user\}\}'s position\./);
      assert.match(handoffSource, /Keep prose literal, physically clear, and grounded\./);
      assert.match(handoffSource, /Write cohesive scene beats\./);
      assert.match(handoffSource, /Do not break one beat into staccato action stacking or body-cue pileups\./);
      assert.match(handoffSource, /#3 - OUTPUT/);
      assert.match(handoffSource, /Begin with BEGIN_FINAL_NARRATION and end with END_FINAL_NARRATION\./);
      ...
      assert.ok(
        narrativeContractSource.indexOf('formatNarrativeFacts({ summary, handoff, resolution, ledger, options })') < narrativeContractSource.indexOf('renderControlEngineNarrativeContract()'),
        'narrativeFacts should be inserted before the renderControlEngine reminder in formatNarrativeContract.',
      );
      assert.ok(
      ...
      assert.ok(
        narrativeContractSource.indexOf('renderFinalWritingStyleReminder(options)') < narrativeContractSource.indexOf("'#3 - OUTPUT'"),
        'writing style reminder should appear before the output contract.',
      );
      assert.match(handoffSource, /embodiedPerception:/);
      assert.match(handoffSource, /strictEpistemology:/);
      assert.match(handoffSource, /characterTurnPacing:/);
      const handoffCharacterTurnPacing = handoffSource.slice(
        handoffSource.indexOf('characterTurnPacing:'),
        handoffSource.indexOf('`;', handoffSource.indexOf('characterTurnPacing:')),
      );
      assert.match(handoffCharacterTurnPacing, /Execute characterTurnPacing\(response, context\)/);
      assert.match(handoffCharacterTurnPacing, /Each active NPC gets 1 paragraph-turn per \{\{user\}\} input\./);
      assert.match(handoffCharacterTurnPacing, /No second turn, repeated body language, or extra micro-actions outside the NPC A -> NPC B -> NPC A exception\./);
      assert.doesNotMatch(handoffCharacterTurnPacing, /Surrounding scene detail/);
      assert.doesNotMatch(handoffCharacterTurnPacing, /current interaction and active participants/);
      assert.match(handoffSource, /linearChronology:/);
      assert.match(handoffSource, /agencySeparation:/);
      assert.match(handoffSource, /activeHandoff:/);
      assert.match(handoffSource, /Begin with the immediate consequence of \{\{user\}\}'s latest input/i);
      assert.match(handoffSource, /Do not echo, repeat, summarize, or recap any part of \{\{user\}\}'s latest input\./);
      assert.match(handoffSource, /Do not insert undeclared intermediate actions\./);
      assert.match(handoffSource, /\{\{user\}\} controls the protagonist\. The narrator controls the world and consequences\./);
      assert.match(handoffSource, /Do not write \{\{user\}\}'s speech, thoughts, feelings, choices/);
      assert.match(handoffSource, /End on a natural beat that \{\{user\}\} can immediately respond to\./i);
      assert.match(handoffSource, /Do not continue past the handoff beat\./i);
  ...
  },
  {
    ...
    run() {
      ...
      assert.match(source, /A sensory violation is a prose-rule violation/);
      assert.match(source, /A chronology violation is a prose-rule violation/);
      ...
      assert.match(source, /Repair smell\/taste only when it is clearly used for "the air," atmosphere, mood, romance/);
      assert.match(source, /Do not add a new smell or taste to replace a removed one/);
      assert.match(source, /PASS 3: linearChronology\(response, RECENT_USER_INPUT\)/);
      assert.match(source, /PASS 4: activeHandoff\(response\)/);
      assert.match(source, /PASS 5: integrityCheck\(original, corrected\)/);
  ...
  },
  {
    ...
    run() {
      ...
      const combinedIndex = source.indexOf('const combinedRaw = await requestCombinedPostNarrationPass({');
      ...
      assert.match(source, /The tracker delta must be based on CORRECTED_NARRATION/);
      assert.match(source, /BEGIN_CORRECTED_NARRATION/);
      assert.match(source, /END_CORRECTED_NARRATION/);
      ...
      assert.match(source, /function armNarratorDepthReplay[\s\S]*setNarratorDepthPrompt\(context, narratorModelContext\)/);
      assert.match(source, /if \(isNarratorDepthReplayPromptPass\(\)\) \{[\s\S]*beginProseGuardDisplayIntercept\(state\.pendingGeneration\.type \|\| 'normal'\)/);
      assert.match(source, /replacePromptWithReplayNotice\(eventData\.chat\)/);
      assert.doesNotMatch(source, /appendNarratorContextToPrompt/);
  ...
  },
  {
    ...
    run() {
      ...
      assert.match(source, /# ABILITIES: exactly \$\{PROGRESSION_REQUIRED_ABILITIES\} activated non-spell abilities/);
      assert.match(source, /ordinary PHY\/MND\/CHA action mechanics would not already cover/);
      assert.match(source, /Good ability shapes include Shadow Step, Voice Projection, Telepathy, Wall Cling, Heat Sight, Mist Form, Amphibious Shift/);
      ...
      assert.match(source, /Allowed spell categories are offensive magic, practical utility magic, traversal, environmental manipulation, and healing/);
  ...
  },
  {
    ...
    run() {
      ...
      assert.match(source, /Narrate \{\{user\}\}\\?'s final moments on Earth first/);
      assert.match(source, /This final moment should be brief, concrete, external, and no more than one short paragraph/);
      ...
      assert.match(source, /formatAdventureIntroNarratorModelPromptContext\(adventurePrompt, state\.pendingGeneration\)/);
      ...
      assert.match(introBypassSource, /formatAdventureIntroNarratorPromptContext\(adventurePrompt, state\.pendingGeneration\)/);
      assert.match(introBypassSource, /formatAdventureIntroNarratorModelPromptContext\(adventurePrompt, state\.pendingGeneration\)/);
      assert.match(introBypassSource, /const pendingRun = buildAdventureIntroPendingRun\(context, state\.pendingGeneration, narratorModelContext\)/);
      assert.match(introBypassSource, /armNarratorDepthReplay\(\{\s*context,\s*pendingGeneration: state\.pendingGeneration,\s*pendingRun,\s*narratorContext,\s*narratorModelContext,\s*generationMode,\s*spacingLabel: 'adventure intro model call'/);
      assert.match(introBypassSource, /sanitizeFinalPromptHistory\(eventData\.chat\)/);
      assert.match(introBypassSource, /replacePromptWithReplayNotice\(eventData\.chat\)/);
      assert.match(introBypassSource, /abortGenerationAfterPromptReady\(context\)/);
      assert.match(introBypassSource, /scheduleNarratorDepthReplay\(\)/);
      assert.doesNotMatch(introBypassSource, /appendNarratorContextToPrompt|beginProseGuardDisplayIntercept|markNextNarratorRequestThinkingDisabled|waitForStoryEngineModelCallSpacing/);
  ...
  },
  {
    ...
    run() {
      ...
      assert.match(source, /async function requestProgressionText[\s\S]*return await context\.generateRawData\(\{ prompt: textPrompt, responseLength, \.\.\.overridePayload \}\)/);
