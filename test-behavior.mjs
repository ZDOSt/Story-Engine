...
const tests = [
  ...
  },
  {
    ...
    run() {
      ...
      assert.doesNotMatch(source, /const TRACKER_WIDGET_MAX_HEIGHT = 900/);
      ...
      assert.deepEqual(migrated, {
        ...
        trackerWidgetLayoutMigrationVersion: 2,
      });
      ...
      const repairedCurrent = { trackerWidgetHeight: 0, trackerWidgetWidth: null, trackerWidgetLayoutMigrationVersion: 2 };
  ...
  },
  {
    ...
    run() {
      ...
      assert.match(coAuthorText, /Do not introduce a new goal, unrelated topic, consequential choice, stakes-bearing action, or mechanically meaningful effect/);
      ...
      assert.match(inlineCoAuthorText, /ACTIVE INLINE CO-AUTHOR SCOPE for \{\{user\}\} is valid for this response only/);
      assert.match(inlineCoAuthorText, /Render authorized actions as vivid, specific in-scene prose integrated with surrounding narration/);
  ...
  },
  {
    ...
    run() {
      ...
      assert.match(runnerSource, /semantic pass owns the referent classification/i);
      assert.match(runnerSource, /const itemUse = normalizeItemUseForHandoff\(/);
      ...
      const itemLedgerWithTargets = (itemUse, identifyTargets) => {
        const ledger = itemLedger(itemUse);
        ledger.resolutionEngine.identifyTargets = {
          ...ledger.resolutionEngine.identifyTargets,
          ...identifyTargets,
        };
        return ledger;
      ...
      const omittedGuitarReport = runCase({
        ...
        ledger: itemLedger(emptyItemUse()),
      });
      assert.deepEqual(omittedGuitarReport.finalNarrativeHandoff.resolutionPacket.ItemUse, {
        Attempted: 'N',
        Available: 'N',
        Item: '(none)',
        Source: 'none',
      ...
      const unavailableVaseReport = runCase({
        ...
        ledger: itemLedger({
          ...
          item: 'vase',
          source: 'unavailable',
          evidence: 'the latest input treats the vase as an object to possess',
          noEffectReason: 'no valid source establishes a vase',
        }),
      });
      assert.equal(unavailableVaseReport.finalNarrativeHandoff.resolutionPacket.ItemUse.Attempted, 'Y');
      assert.equal(unavailableVaseReport.finalNarrativeHandoff.resolutionPacket.ItemUse.Item, 'vase');
      assert.equal(unavailableVaseReport.finalNarrativeHandoff.resolutionPacket.ItemUse.Available, 'N');
      assert.equal(unavailableVaseReport.finalNarrativeHandoff.resolutionPacket.ItemUse.Source, 'unavailable');
      assert.match(prompt(unavailableVaseReport), /The item must not appear, work/i);

      const livingTargetMisclassifiedReport = runCase({
        userText: 'I embrace Phoebe.',
        ledger: itemLedgerWithTargets({
          ...emptyItemUse(),
          attempted: true,
          item: 'Phoebe',
          source: 'unavailable',
          evidence: 'semantic item classification',
        }, {
          ActionTargets: ['Phoebe'],
        }),
      });
      assert.deepEqual(livingTargetMisclassifiedReport.finalNarrativeHandoff.resolutionPacket.ItemUse, {
        Attempted: 'N',
        Available: 'N',
        Item: '(none)',
        Source: 'none',
      ...
      const anatomyMisclassifiedReport = runCase({
        userText: "I slap Phoebe's ass.",
        ledger: itemLedgerWithTargets({
          ...emptyItemUse(),
          attempted: true,
          item: 'ass',
      ...
      assert.deepEqual(anatomyMisclassifiedReport.finalNarrativeHandoff.resolutionPacket.ItemUse, {
        Attempted: 'N',
        Available: 'N',
        Item: '(none)',
        Source: 'none',
      ...
      const gearGuitarReport = runCase({
        ...
        ledger: itemLedger({ ...emptyItemUse(), attempted: true, item: 'guitar', source: 'gear', evidence: 'guitar in saved gear' }),
      });
      ...
      const inventoryGuitarReport = runCase({
        ...
        ledger: itemLedger({ ...emptyItemUse(), attempted: true, item: 'guitar', source: 'inventory', evidence: 'guitar in saved inventory' }),
      });
      ...
      const quotedCommandReport = runCase({
        ...
        ledger: itemLedger({ ...emptyItemUse(), attempted: true, item: 'sword', source: 'ambient', evidence: 'semantic hallucination' }),
      });
      ...
      const groundedEquivalentVerbReport = runCase({
        ...
        userState: { gear: ['sword'], inventory: [] },
        ledger: itemLedger({ ...emptyItemUse(), attempted: true, available: true, item: 'sword', source: 'gear', evidence: 'sword in gear' }),
      });
      ...
      const dialogueClaimDoesNotEstablishReport = runCase({
        ...
        ledger: itemLedger({ ...emptyItemUse(), attempted: true, available: true, item: 'guitar', source: 'scene', evidence: 'Alice claimed a guitar was present' }),
      });
  ...
  {
    name: '48b semantic response budget uses only confirmed NPCs in the current exchange',
    run() {
      const ordinaryScene = [
        {
          role: 'assistant',
          content: 'Phoebe heads home. She returns shortly. Something catches your attention. Her mother remains away. Whatever happens next is unclear.',
        },
        { role: 'user', content: 'I follow her to see where she is going.' },
      ];
      assert.equal(
        estimateSemanticResponseLength({}, ordinaryScene, { assembledPrompt: true }),
        4096,
        'Untracked capitalized prose must not be counted as NPCs.',
      );

      const currentTrackedScene = [
        { role: 'assistant', content: 'Phoebe speaks with Mara beside the gate.' },
        { role: 'user', content: 'I ask Phoebe and Mara what happened.' },
      ];
      ...
      assert.equal(
        estimateSemanticResponseLength(trackerSnapshot, currentTrackedScene, { assembledPrompt: true }),
        4096 + (2 * 768),
        'Only exact confirmed tracker names in the current exchange should increase the budget.',
      );

      const staleTrackedScene = [
        { role: 'assistant', content: 'Offscreen departed yesterday.' },
        { role: 'user', content: 'I watched them leave.' },
        { role: 'assistant', content: 'Phoebe closes the gate.' },
        { role: 'user', content: 'I speak to Phoebe.' },
      ];
      assert.equal(
        estimateSemanticResponseLength(trackerSnapshot, staleTrackedScene, { assembledPrompt: true }),
        4096 + 768,
        'Tracked NPCs outside the current exchange must not inflate the current response budget.',
      );

      const crowdedTracker = Object.fromEntries(
        ['Ari', 'Bela', 'Cato', 'Dara', 'Eris', 'Fenn'].map(name => [name, {}]),
      );
      const crowdedScene = [
        { role: 'assistant', content: 'Ari, Bela, Cato, Dara, Eris, and Fenn gather around you.' },
        { role: 'user', content: 'I address Ari, Bela, Cato, Dara, Eris, and Fenn.' },
      ];
      assert.equal(
        estimateSemanticResponseLength(crowdedTracker, crowdedScene, { assembledPrompt: true }),
        8192,
        'Confirmed scene complexity must remain bounded by the provider-safe ceiling.',
      );
    },
  },
  {
    name: '48b.1 old text completion hook is removed',
    run() {
      const source = fs.readFileSync(new URL('index.js', import.meta.url), 'utf8');
  ...
  },
  {
    ...
    run() {
      ...
      }), { collapsed: true, x: 1140, y: 120, width: 36, height: 36, panelLeft: 0, panelTop: 0 });
      assert.deepEqual(getLayout({
      ...
      assert.deepEqual(malformed, {
        ...
        narratorHandoffWidgetLayoutMigrationVersion: 1,
      });
  ...
  },
  {
    ...
    run() {
      ...
      const geometrySource = source.slice(
        source.indexOf('function clampTrackerWidgetHeight'),
        source.indexOf('function getTrackerWidgetLayoutFromAnchor'),
      );
      ...
      assert.deepEqual(geometry.resizeTrackerWidgetFromCorner(start, 'bottom-right', 100, 50), {
