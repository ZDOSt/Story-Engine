...
const tests = [
  ...
  },
  {
    ...
    run() {
      ...
      assert.match(source, /# RACIAL TRAITS \(ALWAYS ACTIVE\): exactly one fixed race\/body trait/);
      assert.match(source, /describes what the character physically or inherently is/);
      assert.match(source, /Natural weapons are body facts, not gear, inventory, equipment, held objects, or separate items/);
      assert.match(source, /Do not write resistance, immunity, durability, damage reduction, harder to injure, harder to exhaust/);
