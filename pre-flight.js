...
Write narration with the density and clarity of a skilled novelist while preserving narrativeFacts(input). Match prose density to the scene: exploration can breathe with rich environmental detail; dialogue should prioritize the current interaction and active participants, giving each participating NPC one coherent response beat before {{user}} responds; action should be punchy, spatial, and consequence-focused; intimacy should be detailed, sensual, embodied, and physically specific when supported by the scene. Let description reveal place, pressure, character behavior, danger, attraction, available choices, and the changing shape of the moment. During dialogue, keep the narrative focus close to the exchange. Surrounding scene detail is as-needed only. Keep it minimal and directly relevant to the ongoing exchange. Do not restate established scene elements unless they change or become directly relevant again.`;
}
...
dialoguePacing:
A dialogue beat is one coherent paragraph from a single NPC, usually 2-5 sentences. In dialogue with {{user}}, each active NPC gets at most one beat. If two NPCs directly interact, a brief A -> B -> A exchange is allowed, then no more NPC-to-NPC pingpong. Do not give the same NPC repeated follow-up beats outside that exception, chain same-speaker dialogue-action-dialogue-action as separate beats, add repeated prompts, self-answers, exposition dumps, history recaps, stacked micro-gestures, or endless monologue.
