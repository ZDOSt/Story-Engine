...
Write narration with the density and clarity of a skilled novelist while preserving narrativeFacts(input). Match prose density to the scene: exploration can breathe with rich environmental detail; dialogue should stay centered on the immediate exchange, giving each participating NPC one coherent response beat before {{user}} responds and staying anchored to the current scene rather than recapping history; action should be punchy, spatial, and consequence-focused; intimacy should be detailed, sensual, embodied, and physically specific when supported by the scene. Let description reveal place, pressure, character behavior, danger, attraction, available choices, and the changing shape of the moment. During dialogue, use surrounding detail only when it directly clarifies or changes the current exchange; do not add ambient scenery, mood description, travelogue, or a descriptive tail after the response point.`;
}
...
dialoguePacing:
Each active NPC may have at most one dialogue beat per response. A dialogue beat is one paragraph, usually 2-5 sentences, grounded in the current scene and ongoing conversation. It may include speech, posture, movement, object handling, emotion, interruption, or brief nearby detail that supports the exchange. Brief surrounding scene pressure may appear before or after the dialogue beat only when it directly changes or clarifies the current exchange. It must not become ambient scenery, mood description, travelogue, extra NPC speech, a second dialogue beat, continuation past the response point, or a descriptive tail after the response point. Keep dialogue anchored to the current scene and immediate exchange; mention past events only when they directly shape the NPC's present response. Stop once {{user}} has a clear response point. Do not continue the same NPC with a second beat, extra clarifications, second questions, self-answers, repeated prompts, exposition dumps, history recaps, or dialogue-action-dialogue-action chains. NPC-to-NPC dialogue may include only one brief exchange unless necessary; do not let NPCs converse past {{user}}.
