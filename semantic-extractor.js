...
const ITEM_USE_REFERENT_RULE = [
    ...
    'For Attempted=Y, decide naturalAvailability from the established current setting and scene context. Before deciding, ask internally: "Could this specific item be naturally available in this setting?" Use yes only for an ordinary, low-consequence object/material that plausibly belongs in this setting; use no when the setting conflicts with it; use unknown when the setting or item status is insufficiently established.',
    'A normal household object may be naturally available in a home or similar setting; a natural outdoor material may be naturally available outdoors. Do not treat a rare, valuable, magical, specialized, owned, plot-relevant, or otherwise unusual item as naturally available. Those require an authoritative saved gear/inventory/current-scene/prior-scene source. Do not use naturalAvailability to decide ownership, access difficulty, possession transfer, action success, mechanics, or narration.',
    'naturalAvailability must be exactly one of yes, no, or unknown. Use unknown when Attempted=N. The extension deterministically resolves source, final availability, evidence, and any unavailable-item consequence after this pass.',
    'Return only the semantic item decision: Attempted, Item, and naturalAvailability. Do not output Available, Source, Evidence, or NoEffectReason for itemUse.',
].join(' ');
...
async function generateSemanticToolCallWithProfile(prompt, responseLength, options = {}) {
    ...
    const overridePayload = {
        ...
        ...(String(options.semanticModel || '').trim() ? { model: String(options.semanticModel).trim() } : {}),
...
function buildSemanticPreflightSchema() {
    ...
    const itemUse = object({
        ...
        naturalAvailability: enumString(ITEM_NATURAL_AVAILABILITY, 'Interpret the current setting and scene. yes only when this specific ordinary, low-consequence item/material could naturally be available here; no when the setting conflicts with it; unknown when insufficiently established. A normal household object may be natural in a home; a natural outdoor material may be natural outdoors. Rare, valuable, magical, specialized, owned, plot-relevant, or unusual items require an authoritative saved/source entry and must not receive yes. Use unknown when Attempted=N. This field does not decide ownership, access difficulty, possession transfer, success, or mechanics.'),
    });
...
const SEMANTIC_FIELD_GUIDANCE = [
    ...
    '- ResolutionEngine.itemUse applies the ITEM_USE_REFERENT_RULE before any availability check. It activates only for a direct user interaction with one specifically identified concrete object/material. Searching, scanning, looking around, inspecting, examining, rummaging, foraging, or seeking something/anything useful is not itemUse and must not receive an unavailable-item branch. Generic categories such as weapon, tool, object, item, something, or anything are not Item values. Return only Attempted, Item, and naturalAvailability for this semantic decision; do not output Available, Source, Evidence, or NoEffectReason because the extension resolves those from authoritative saved state plus this scene-aware semantic judgment. Item interaction never grants ownership or updates inventory by itself.',
...
function buildSemanticContractText(userName, charName, type, trackerSnapshot, playerTrackerSnapshot = {}, options = {}) {
    ...
    return `Active names: user=${userName}, character=${charName}\nGeneration type=${type || 'normal'}\nNPC tracker snapshot JSON:\n${JSON.stringify(trackerSnapshot, null, 2)}\nPlayer tracker snapshot JSON:\n${JSON.stringify(semanticPlayerTrackerSnapshot, null, 2)}\n\n` +
        ...
        `${ITEM_USE_REFERENT_RULE} Return only Attempted, Item, and naturalAvailability for itemUse. Do not add Available, Source, Evidence, or NoEffectReason; the extension resolves item presence, availability, source, evidence, and any unavailable-item consequence after the semantic pass. ` +
        ...
        'The ITEM_USE_REFERENT_RULE is authoritative. The model decides whether a direct concrete item interaction was attempted, which item it names, and whether that item could naturally be available in the current setting. The extension decides authoritative source, ownership, final availability, and item-dependent gating. Leave unrelated actions, dialogue, movement, targets, and relationships independent. ' +
...
function validateRawLedgerContract(ledger, raw) {
    ...
    if (!ITEM_NATURAL_AVAILABILITY.includes(ledger?.resolutionEngine?.itemUse?.naturalAvailability)) missing.push('resolutionEngine.itemUse.naturalAvailability');
...
function normalizeItemUse(value) {
    ...
    const naturalAvailability = ITEM_NATURAL_AVAILABILITY.includes(cleanScalar(source.naturalAvailability ?? source.NaturalAvailability).toLowerCase())
        ? cleanScalar(source.naturalAvailability ?? source.NaturalAvailability).toLowerCase()
        : 'unknown';
    ...
    return {
        ...
        naturalAvailability: attempted ? naturalAvailability : 'unknown',
        source: itemSource,
        evidence: attempted && !isNoneValue(evidence) ? evidence : '(none)',
        noEffectReason: attempted && !available && !isNoneValue(noEffectReason) ? noEffectReason : '(none)',
    };
