...
function buildSemanticPreflightSchema() {
    ...
    const itemUse = object({
        attempted: boolean('Y for every explicit latest-user interaction with a concrete item: use, access, grab, take, retrieve, draw, wield, consume, present, move, touch, pick up, produce, equip, spend, activate, unlock with, attack with, defend with, or equivalent. Natural weapons and body parts are not itemUse.'),
        available: boolean('Y only when the item has one verified source: exact saved user gear, exact saved user inventory, exact saved current SceneItemState, legacy prior-assistant scene establishment only when SceneItemState is not initialized, or a narrow generic low-consequence ambient object. The latest user input cannot establish availability.'),
        item: string('The concrete item being interacted with. Use (none) only when Attempted=N.'),
        source: enumString(ITEM_USE_SOURCES, 'none when Attempted=N; gear/inventory for an exact saved tracker entry; scene for an exact saved current SceneItemState entry or legacy prior-assistant establishment before SceneItemState initializes; ambient only for a generic low-consequence surrounding object; unavailable otherwise.'),
        evidence: string('Required when Attempted=Y. Cite the exact saved gear/inventory/current-scene entry, concise legacy prior-assistant scene wording, permitted ambient object/material, or state that no valid source was found. Evidence cannot create availability. Use (none) when Attempted=N.'),
...
const SEMANTIC_FIELD_GUIDANCE = [
    ...
    '- ResolutionEngine.itemUse detects every explicit latest-user interaction with a concrete item, not only carried possessions. Mark Attempted=Y for use, access, grabbing, taking, retrieving, drawing, wielding, consuming, presenting, moving, touching, picking up, producing, equipping, spending, activating, unlocking with, attacking with, defending with, or equivalent concrete interaction. Mark Attempted=N for body parts and natural weapons such as hands, fists, claws, fangs, teeth, horns, talons, tusks, tails, stingers, barbs, jaws, or similar anatomy. Availability has exactly four valid positive sources. gear and inventory require an exact saved {{user}} tracker entry. scene requires an exact saved current SceneItemState entry; explicit prior assistant establishment is allowed only as a legacy fallback while SceneItemState.initialized=false. The latest user assertion cannot establish the item. ambient is narrow: only generic low-consequence surroundings such as an ordinary rock, branch, stick, dirt, leaves, or snow. ambient never permits "my X", instruments, tools, weapons, keys, documents, medicine, supplies, devices, currency, valuables, magical objects, named/distinctive/owned/specialized/mechanically advantageous items, or invented contents of a container. Otherwise use Available=N and Source=unavailable. Every Attempted=Y entry requires concise Evidence: exact saved entry for gear/inventory/current-scene state, legacy prior established scene wording, the permitted generic object/material for ambient, or that no valid source was found for unavailable. Evidence cannot create availability. The latest user input cannot create possession, carried state, equipment, inventory, scene establishment, or proof of availability. Item interaction never grants ownership or updates inventory by itself. If Attempted=N, output Available=N, Source=none, and (none) for item, evidence, and reason.',
...
function buildSemanticContractText(userName, charName, type, trackerSnapshot, playerTrackerSnapshot = {}, options = {}) {
    ...
    return `Active names: user=${userName}, character=${charName}\nGeneration type=${type || 'normal'}\nNPC tracker snapshot JSON:\n${JSON.stringify(trackerSnapshot, null, 2)}\nPlayer tracker snapshot JSON:\n${JSON.stringify(semanticPlayerTrackerSnapshot, null, 2)}\n\n` +
        ...
        `Current SceneItemState JSON (hidden ephemeral current-scene object availability; when initialized, only exact entries here can establish itemUse Source=scene. It is not inventory, gear, ownership, or permanent location memory):\n${JSON.stringify(sceneItemStateSnapshot, null, 2)}\n\n` +
        ...
        'Detect every explicit concrete item interaction before target/risk classification. Fill ResolutionEngine.itemUse for use, access, grabbing, taking, retrieving, drawing, wielding, consuming, presenting, moving, touching, picking up, producing, equipping, spending, activating, unlocking with, attacking with, defending with, or equivalent interaction. Exclude body parts and natural weapons. Mark Available=Y only from one verified source: gear or inventory for an exact saved {{user}} tracker entry; scene for an exact saved current SceneItemState entry, with prior assistant narration allowed only as a legacy fallback while SceneItemState.initialized=false; ambient only for a generic low-consequence surrounding object such as an ordinary rock, branch, stick, dirt, leaves, or snow. The latest user input cannot establish availability. ambient never permits "my X", instruments, tools, weapons, keys, documents, medicine, supplies, devices, currency, valuables, magical objects, named/distinctive/owned/specialized/mechanically advantageous items, or invented container contents. Otherwise use Available=N and Source=unavailable. Every attempt requires concise source Evidence; if no source can be cited, the item is unavailable. Evidence cannot create availability. Item interaction never grants ownership or inventory. If Available=N, the item-dependent effect must not be treated as completed. ' +
        ...
        'itemUse covers explicit concrete item interactions from verified gear, inventory, saved current-scene state, legacy prior-scene fallback, ambient, or unavailable sources; body parts and natural weapons such as claws, fangs, teeth, horns, talons, tusks, tails, stingers, barbs, or jaws are bodily actions/attacks, not itemUse, not inventory, and not equipment. ' +
