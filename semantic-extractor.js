...
function buildSemanticPreflightSchema() {
    ...
    const itemUse = object({
        ...
        available: boolean('Y only when the item has one verified source: exact saved user gear, exact saved user inventory, explicit prior assistant scene establishment, or a narrow generic low-consequence ambient object. The latest user input cannot establish availability.'),
        item: string('The concrete item being interacted with. Use (none) only when Attempted=N.'),
        source: enumString(ITEM_USE_SOURCES, 'none when Attempted=N; gear/inventory for an exact saved tracker entry; scene for explicit prior assistant establishment; ambient only for a generic low-consequence surrounding object; unavailable otherwise.'),
        evidence: string('Required when Attempted=Y. Cite the exact saved gear/inventory entry, concise prior assistant scene wording, permitted ambient object/material, or state that no valid source was found. Evidence cannot create availability. Use (none) when Attempted=N.'),
...
function buildSemanticContractText(userName, charName, type, trackerSnapshot, playerTrackerSnapshot = {}, options = {}) {
    ...
    return `Active names: user=${userName}, character=${charName}\nGeneration type=${type || 'normal'}\nNPC tracker snapshot JSON:\n${JSON.stringify(trackerSnapshot, null, 2)}\nPlayer tracker snapshot JSON:\n${JSON.stringify(semanticPlayerTrackerSnapshot, null, 2)}\n\n` +
        ...
        'itemUse covers explicit concrete item interactions from verified gear, inventory, prior-scene, ambient, or unavailable sources; body parts and natural weapons such as claws, fangs, teeth, horns, talons, tusks, tails, stingers, barbs, or jaws are bodily actions/attacks, not itemUse, not inventory, and not equipment. ' +
