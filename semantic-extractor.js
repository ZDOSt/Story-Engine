...
const ITEM_USE_REFERENT_RULE = [
    ...
    'Scan every explicit treatment of a candidate referent in the latest user input, regardless of wording, verb, or construction.',
    'First classify the referent, then apply availability.',
    'Attempted=Y only when the referent is an independently existing inanimate object or material that can be present, accessed, possessed, or affected in the scene.',
    ...
    'If the referent is an item but no valid source establishes it, preserve the attempt as Attempted=Y and set Available=N, Source=unavailable; the latest user wording cannot establish presence or possession.',
].join(' ');
...
function buildSemanticPreflightSchema() {
    ...
    const itemUse = object({
        attempted: boolean('Classify every referent explicitly treated by the latest user input, independent of wording, verb, or construction. Y only when the referent is an independently existing inanimate object or material. Living beings, creatures, anatomy/body parts, natural weapons, bodily contact or poses, movements, locations, surfaces, sensations, thoughts, dialogue, events, relationships, and abstract concepts are not itemUse. An unusual, intimate, indirect, possessive, or unconventional interaction does not change this boundary.'),
        available: boolean('After Attempted=Y and only after the referent is classified as an item, set Y only when the item has one verified source: exact saved user gear, exact saved user inventory, exact saved current SceneItemState, legacy prior-assistant scene establishment only when SceneItemState is not initialized, or a narrow generic low-consequence ambient object. The latest user input cannot establish availability.'),
...
function buildSemanticContractText(userName, charName, type, trackerSnapshot, playerTrackerSnapshot = {}, options = {}) {
    ...
    return `Active names: user=${userName}, character=${charName}\nGeneration type=${type || 'normal'}\nNPC tracker snapshot JSON:\n${JSON.stringify(trackerSnapshot, null, 2)}\nPlayer tracker snapshot JSON:\n${JSON.stringify(semanticPlayerTrackerSnapshot, null, 2)}\n\n` +
        ...
        `${ITEM_USE_REFERENT_RULE} After classification, apply availability exactly as follows: gear or inventory requires an exact saved {{user}} tracker entry; scene requires an exact saved current SceneItemState entry, with prior assistant narration allowed only as a legacy fallback while SceneItemState.initialized=false; ambient allows only a generic low-consequence surrounding object/material. The latest user input cannot establish availability. Ambient does not establish owned, specialized, valuable, magical, weapon, tool, key, document, medicine, supply, device, currency, named/distinctive, or invented container-content claims. Otherwise use Available=N and Source=unavailable. Every Attempted=Y entry requires concise source Evidence; Evidence cannot create availability. Item interaction never grants ownership or inventory. If Available=N, block only the item-dependent effect; unrelated actions, dialogue, movement, targets, and relationships remain independent. ` +
