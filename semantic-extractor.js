...
function buildSemanticContractText(userName, charName, type, trackerSnapshot, playerTrackerSnapshot = {}, options = {}) {
    ...
    return `Active names: user=${userName}, character=${charName}\nGeneration type=${type || 'normal'}\nNPC tracker snapshot JSON:\n${JSON.stringify(trackerSnapshot, null, 2)}\nPlayer tracker snapshot JSON:\n${JSON.stringify(playerTrackerSnapshot, null, 2)}\n\n` +
        ...
        'Execute ResolutionEngine(input) semantic functions in order: identifyGoal, identifyChallenge, userAbilityUse, itemUse, claimCheck, stealthContest, identifyTargets, activeHostileThreat, classifyPhysicalBoundaryPressure, intimacyAdvanceExplicit, boundaryViolationExplicit, harmMode, rollNeeded, rollReason, actionBucket, socialBucket, combatType, actionUnits, environmentDifficultyTier, genStats. Copy those outputs into the ResolutionEngine lines using the exact function/key names shown in the template. ' +
...
function parseCompactLedger(text, trackerSnapshot) {
    ...
    const derivedActionMarkers = deriveActionMarkersFromUnits(parsedActionUnits, actionBucket);
    ...
    const resolutionEngine = {
        ...
        classifyHostilePhysicalIntent: actionBucket === 'Combat',
...
function normalizeLedger(ledger) {
    ...
    ledger.resolutionEngine.classifyHostilePhysicalIntent = ledger.resolutionEngine.actionBucket === 'Combat';
    ...
    delete ledger.resolutionEngine.nonLethal;
...
function deriveActionMarkersFromUnits(units, actionBucket = 'None') {
