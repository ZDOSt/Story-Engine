...
function buildSemanticPreflightSchema() {
    ...
    const powerActorAssessmentSchema = {
        ...
        properties: {
            ...
            isPowerActor: {
                type: 'boolean',
                description: 'Y when the entity has means to reach beyond personal action: money, influence, authority, status, agents, resources, institution/faction access, reputation, information, territory, magic, command, or leverage. Assess semantically, not by title keyword.',
            },
            actorType: { type: 'string' },
...
const COMPACT_LEDGER_CONTRACT = [
    ...
    '- ResolutionEngine.identifyTargets.PowerActors is strategic-only: list organizations, factions, institutions, groups, and potential power figures with means to reach beyond personal action: money, influence, authority, status, agents, resources, institution/faction access, reputation, information, territory, magic, command, or leverage. PowerActors never create rolls, NPCInScene, RelationshipEngine, B/F/H, injuries, or visible tracker entries by themselves.',
    ...
    '- PowerActorEnmity is hidden power-actor memory. First assess entities from ResolutionEngine.identifyTargets.PowerActors semantically, not by keyword/title: an entity is a power actor when it has means to reach beyond personal action, such as money, influence, authority, status, agents, resources, institution/faction access, reputation, information, territory, magic, command, leverage, trade reach, hired help, or social power. Explicit prominence, wealth, rank, office, ownership, command, fame, backing, network access, or unusual resources are enough for a power-actor assessment unless context clearly limits them to ordinary personal reaction. Ordinary people with only personal reaction are not power actors even if they have a job title.',
    '- PowerActorEnmity.assessments is audit-only diagnosis. Include one assessment for each meaningful ResolutionEngine.identifyTargets.PowerActors entry and any target/observer NPC whose possible reach should be auditable, including the active character when card/lore/scenario context gives explicit prominence, wealth, authority, backing, network access, or other reach beyond personal action. You may also include an explicitly affected organization/group if it is the relevant power actor behind a targeted NPC. Assessment never creates enmity by itself and never replaces RelationshipEngine. If a living NPC appears in a normal target/observer list, still create the required RelationshipEngine entry even when isPowerActor=Y.',
