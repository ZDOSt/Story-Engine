        ...
        properties: {
            ...
            resolutionEngine: {
                ...
                properties: {
                    ...
                    stealthContest: {
                        type: 'boolean',
                        description: 'Y only when the latest explicit user goal avoids perception/detection by a specific established living detector/opponent, follows/shadows a specific established living entity without being noticed, passes a specific living watcher, or remains unseen/unheard from that detector. N when no specific established living detector/opponent is present. This is never ENV opposition.',
                    },
                    identifyTargets: {
                        type: 'object',
                        additionalProperties: false,
                    ...
                    rollNeeded: {
                        type: 'boolean',
                        description: 'The sole semantic roll gate. Y for fresh unresolved DEF.STAKES_POSITIVE items. N only for DEF.NO_STAKES exclusions when no positive stake is present or when that exact positive stake is already resolved/suppressed. Positive stakes win over ordinary continuity.',
                    },
                    rollReason: {
                        type: 'string',
                    ...
                    environmentDifficultyTier: {
                        ...
                        description: 'Semantic environmental opposition tier only when actionBucket=Challenge and OppTargets.ENV has a non-living obstacle or condition that makes the goal fail-able. Use easy/trivial, average, hard, or extreme; set none for non-Challenge buckets or no ENV roll.',
                    },
...
const COMPACT_LEDGER_CONTRACT = [
    ...
    '- ResolutionEngine.stealthContest is semantic-only. Return Y only when the latest explicit user goal avoids detection/perception by a specific established living entity, follows/shadows a specific established living entity without being noticed, passes a specific living watcher, or remains unseen/unheard from that detector. If stealthContest=Y, put that living detector/opponent in ActionTargets and OppTargets.NPC, and keep OppTargets.ENV=(none). If no specific established living detector/opponent is present, return N; terrain, darkness, cover, distance, crowds, weather, and noise are scene conditions, not stealth opposition.',
    '- ResolutionEngine.rollNeeded is the sole semantic roll gate. Apply DEF.STAKES_POSITIVE and DEF.NO_STAKES separately. Return Y when success/failure of the latest explicit user goal or challenge creates a fresh unresolved positive stake: physical risk, harm, danger, a stealthContest against a specific established living detector/opponent, infiltration, material gain/loss, significant trust/status/authority shift, autonomy/freedom, access, secrets, combat, pursuit, restraint, deception, bargaining, environmental obstacle resolution, or explicit goal advancement/failure. Return N only when no positive stake is present, when a stealth-style action lacks a specific established living detector/opponent and has no separate non-stealth obstacle, or when the exact positive stake is already resolved/suppressed by saved fear/terror, hostility/hatred, persisted intimacy boundary, or repeated resolved same-bucket negative social attempt rules. If positive stakes and ordinary continuity/no-stakes wording both seem relevant, positive stakes win. Failed/resolved Bluff blocks repeated Bluff; failed/resolved Intimidate blocks repeated Intimidate. Bluff does not block a later Intimidate, and Intimidate does not block a later Bluff. Repeated wording, stronger insults, renewed same-bucket threats, rephrased same-bucket bluffs, or theatrical display after refusal/failure are aftermath or escalation, not a fresh social contest. ResolutionEngine.rollReason must briefly explain why rollNeeded is Y or N and must not contradict the flag: rollNeeded=Y requires a fresh unresolved positive stake; rollNeeded=N requires no fresh unresolved positive stake.',
    '- ResolutionEngine.actionBucket is the roll shape, not a stat choice. Use None when rollNeeded=N. Use Social only for fresh unresolved NPC-facing social pressure, with socialBucket=Diplomacy for good-faith persuasion/negotiation/reassurance, Bluff for deception/lying/misleading/material false claims, and Intimidate for threats/coercion/blackmail/fear-based demands. Use Combat for direct violence or harmful supernatural attacks; combatType=Mundane for bodily/weapon/natural-weapon/ordinary force, and SpellOrSupernatural for the primary attack being a spell, power, curse, elemental blast, psychic attack, or similar. Use Challenge for physical/environmental obstacles, stealthContest=Y, escape, chase/pursuit, locks, traps, terrain, weather, barriers, hazards, or non-living opposition. If not Social, socialBucket=None. If not Combat, combatType=None.',
    '- ResolutionEngine.actionCount counts combat attack/effect instances, not sentences or ability names. Count separate attacks/effects even when they share one target, one goal, one sentence, one ability, or one combo, and even when {{user}} gives no number. Repeated same-spell/projectile/strike wording such as "two bolts", "twice", "again", "another", "in succession", "then", "and", or "swing, punch, then knee" must produce separate markers up to the three-action cap. Do not count setup, aiming, focusing, movement, pivoting, defense, or flavor unless that act itself is a separate stakes-bearing attack/effect.',
    '- ResolutionEngine.actionUnits breaks the latest explicit {{user}} attempt into the same mechanically counted actions represented by actionCount. actionUnits.count must match actionCount length after the three-action cap. Each action is short clean wording of that action unit; evidence is brief latest-user-text audit evidence. Each actionUnit must correspond to exactly one counted attack/effect in order; do not merge attack chains or repeated same-ability attacks into one summary unit. actionUnits does not decide success, failure, outcome, injury, counterattack, or narration.',
    '- ResolutionEngine.identifyTargets.ActionTargets is the direct interaction list. Include every living NPC the latest user input directly addresses, speaks to, answers, asks, thanks, reassures, requests, warns, commands, helps, treats, touches, accompanies, follows, trades with, gives something to, receives something from, attacks, restrains, deceives, negotiates with, or otherwise directly interacts with this turn, whether rollNeeded is Y or N. ActionTargets are broader than OppTargets.NPC. If the latest user input directly involves a living NPC, that NPC belongs in ActionTargets even when the scene is ordinary continuity and OppTargets.NPC is (none). If stealthContest=Y, the specific established living detector/opponent must be ActionTargets and OppTargets.NPC because their awareness is the opposition; terrain, darkness, noise, crowds, cover, distance, foliage, or weather are scene conditions, not OppTargets.ENV. Do not include bystanders, background crowds, inferred listeners, offscreen entities, or hostilesInScene-only NPCs unless the latest user input directly involves them.',
    ...
    '- ResolutionEngine.identifyTargets.OppTargets.NPC is narrower: list only living entities directly opposing, contesting, resisting, blocking, defending against, acting as the detector/opponent in stealthContest=Y, or being attacked/challenged by {{user}}\'s current action/challenge. Do not put every enemy in OppTargets.NPC just because they are hostile; use hostilesInScene.NPC for the broader hostile pool.',
...
ResolutionEngine.stealthContest=N
...
function buildSemanticContractText(userName, charName, type, trackerSnapshot, playerTrackerSnapshot = {}, options = {}) {
    ...
    return `Active names: user=${userName}, character=${charName}\nGeneration type=${type || 'normal'}\nNPC tracker snapshot JSON:\n${JSON.stringify(trackerSnapshot, null, 2)}\nPlayer tracker snapshot JSON:\n${JSON.stringify(playerTrackerSnapshot, null, 2)}\n\n` +
        ...
        'stealthContest is semantic-only. Return stealthContest=true only when the latest explicit user goal avoids detection/perception by a specific established living entity, follows/shadows a specific established living entity without being noticed, passes a specific living watcher, or remains unseen/unheard from that detector. If stealthContest=true, put that living detector/opponent in ActionTargets and OppTargets.NPC, and keep OppTargets.ENV empty. If no specific established living detector/opponent is present, return stealthContest=false; terrain, darkness, cover, distance, crowds, weather, and noise are scene conditions, not stealth opposition. ' +
        'rollNeeded is the sole semantic roll gate. Apply DEF.STAKES_POSITIVE and DEF.NO_STAKES separately. Return rollNeeded=true for fresh unresolved positive stakes: success/failure would materially change risk, harm, danger, a stealthContest against a specific established living detector/opponent, infiltration, resources, trust/status/authority, autonomy/freedom, access, secrets, combat, pursuit, restraint, deception, bargaining, environmental obstacles, or explicit goal advancement. Return rollNeeded=false only when no positive stake is present, when a stealth-style action lacks a specific established living detector/opponent and has no separate non-stealth obstacle, when the exact positive stake is already resolved/suppressed by saved fear/terror, hostility/hatred, or persisted intimacy boundary, or when the latest input repeats a resolved same-bucket negative social attempt against the same NPC/goal under unchanged disposition. If positive stakes and ordinary continuity/no-stakes wording both seem relevant, positive stakes win. Failed/resolved Bluff blocks repeated Bluff; failed/resolved Intimidate blocks repeated Intimidate. Bluff does not block a later Intimidate, and Intimidate does not block a later Bluff. Do not treat repeated same-bucket threats, stronger insults, rephrased same-bucket bluffs, renewed same-bucket coercion, or dramatic display after refusal/failure as a fresh social contest. rollReason must be a concise explanation that agrees with rollNeeded: rollNeeded=true requires a fresh unresolved positive stake, and rollNeeded=false requires no fresh unresolved positive stake. ' +
        ...
        'Important classification reminders: Romantic, flirtatious, affectionate, suggestive, sexual, or intimate conversation/contact is not a special roll category and does not create stakes by itself. intimacyAdvanceExplicit is strict permission/boundary classification for actual intimate escalation only: mark it true for explicit kissing, sexual touch, undressing toward intimacy, asking to sleep together/have sex, or accepting a prior explicit NPC intimacy invitation; keep it false for flirting, teasing, vague innuendo, compliments, declarations of love, dates, hand-holding, ordinary affection, or "what did you have in mind" style banter. boundaryViolationExplicit is the romance/boundary trigger for mechanics: mark it true only for clear coercion, threats, force, unwanted contact/restraint after refusal, repeated pressure after refusal, humiliation, blackmail, or ignoring a clear stop/no. User intent is explicit-only: identifyGoal and identifyChallenge must use only the latest user-declared action, request, target, and explicit objective; do not infer unstated goals from NPC fear, hostility, suspicion, likely reaction, context, or what an NPC might assume. Do not carry forward a prior social goal as the current goal after it already failed or resolved; post-failure phrases such as accepting refusal, declaring consequence, or escalating toward violence are aftermath/escalation unless the latest input explicitly creates a new non-social contest or a materially different tactic. classifyHostilePhysicalIntent is true only for direct bodily aggression/control against a living entity; object/space/departure/body-adjacent contests without bodily harm are classifyPhysicalBoundaryPressure, not combat. stealthContest=true requires a specific established living detector/opponent in ActionTargets and OppTargets.NPC; if no such detector/opponent exists, use stealthContest=false and no stealth roll unless a separate non-stealth obstacle creates stakes. Terrain, darkness, cover, distance, crowds, weather, and noise are scene conditions, not stealth opposition. rollNeeded is the roll gate: apply DEF.STAKES_POSITIVE and DEF.NO_STAKES separately; do not roll the same settled disposition reaction again, and do not immediately retry the same failed/resolved negative social bucket against the same target for the same goal, but do roll separate combat, pursuit, restraint, theft, bargaining, materially new deception, access, resources, secrets, stealthContest against a specific living detector/opponent, environmental obstacles, or new leverage. If positive stakes and no-stakes wording both seem relevant, positive stakes win unless that exact positive stake is already resolved. actionBucket is classification only: Social/Diplomacy for good-faith persuasion or negotiation, Social/Bluff for deception or material false claims, Social/Intimidate for threats/coercion/fear demands, Combat/Mundane for bodily/weapon/natural-weapon attacks, Combat/SpellOrSupernatural for primary spell/power attacks, Challenge for physical/environmental obstacles, stealthContest=true, escape, chase/pursuit, locks, traps, terrain, weather, barriers, hazards, or non-living opposition. Do not choose stats, dice, bonuses, margins, or outcomes. For each living NPC, mark stakeChangeByOutcome for each possible outcome strictly by DEF.STAKES_POSITIVE and DEF.NO_STAKES: benefit only if that outcome significantly and concretely improves their stakes; harm if it materially worsens their stakes; otherwise none. Do not mark benefit for compliments, flirting, mood improvement, politeness, ordinary conversation, user self-advancement, successful negotiation for the user, choosing not to harm the NPC, failing to harm the NPC, de-escalation without a concrete NPC gain, or the NPC merely surviving/remaining safe.\n\n' +
...
function validateRawLedgerContract(ledger, raw) {
    ...
    if (typeof ledger?.resolutionEngine?.stealthContest !== 'boolean') missing.push('resolutionEngine.stealthContest:boolean');
...
function parseCompactLedger(text, trackerSnapshot) {
    ...
    const required = [
        ...
        'ResolutionEngine.stealthContest',
    ...
    const resolutionEngine = {
        ...
        stealthContest: readBoolean(fields, 'ResolutionEngine.stealthContest', false),
        identifyTargets: {
            hostilesInScene: {
...
function normalizeLedger(ledger) {
    ...
    ledger.resolutionEngine.stealthContest = toBoolean(ledger.resolutionEngine.stealthContest, false);
