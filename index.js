...
const PLAYER_ADVENTURE_GENRE_FRAMES = Object.freeze({
    ...
    Isekai: 'Start with a short playable opening that clearly belongs to the isekai genre. Narrate {{user}}\'s final moments on Earth first. This final moment should be brief, concrete, external, and no more than one short paragraph. In the new world, make the opening unmistakably isekai rather than generic fantasy. Do not rely on stock genre staging. Invent a specific, original crossing and immediate arrival scenario. The final Earth moment and the first new-world scene should feel linked by a distinct transition, visible aftermath, or scene-specific logic, so the opening feels particular to this crossing rather than interchangeable with a familiar template. Keep that evidence external and in-scene.',
